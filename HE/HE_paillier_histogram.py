...
DEFAULT_MEASUREMENTS_FILE = "hist_measurements.txt"
...
def one_hot(value, buckets):
    if value < 0 or value >= buckets:
        raise ValueError(f"value {value} is outside histogram range [0, {buckets})")
...
def expected_histogram(measurements, buckets):
    histogram = [0] * buckets
...
expected = expected_histogram(measurements, args.buckets)
public_key, private_key = paillier.generate_paillier_keypair(n_length=args.key_bits)

# Client side encryption
start = time.perf_counter()
encrypted_reports = [
    [public_key.encrypt(value) for value in one_hot(measurement, args.buckets)]
    for measurement in measurements
]
encryption_duration = time.perf_counter() - start

raw_ciphertext_size = fixed_ciphertext_size_bytes(public_key)
raw_report_size = raw_ciphertext_size * args.buckets

start = time.perf_counter()
serialized_report_sizes = [
    sum(serialized_ciphertext_size_bytes(value) for value in report)
...
start = time.perf_counter()
encrypted_histogram = [
    sum(report[bucket] for report in encrypted_reports)
...
print_metric("raw_ciphertext_bytes_per_ciphertext", raw_ciphertext_size)
...
with open("results_eval_HE_hist.csv", "a") as out:
    # nb_reports,client_report_build_seconds/report,serialized_report_bytes/report,raw_ciphertext_bytes_report,key_size
    out.write(f"{len(measurements)},{(encryption_duration + serialization_duration)/len(measurements):.6f},{max(serialized_report_sizes)},{raw_report_size},{args.key_bits}\n")
